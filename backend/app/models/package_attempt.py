...
class PackageAttempt(Base):
    ...
    # Gameplay score breakdown (for Memory Card and similar games)
    # score_gameplay_base: points from core gameplay (e.g., matching pairs)
    # score_gameplay_bonus: bonus points from efficiency (e.g., time bonus)
    score_gameplay_base: Mapped[float | None] = mapped_column(Float, nullable=True)
    score_gameplay_bonus: Mapped[float | None] = mapped_column(Float, nullable=True)
